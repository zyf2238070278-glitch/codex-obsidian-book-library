...
当前 Codex 项目配置仍从 `/Users/zhaoyunfei/Documents/Codex/2026-07-12/wo` 启动书库服务。双引擎 OCR 新版代码位于 `codex/multi-engine-ocr` 工作分支，因此当前会话实际运行的仍是旧版单引擎实现。旧版会把页面像素上限或 Vision 边界框异常直接记为整本任务失败，没有进入 RapidOCR 回退。
