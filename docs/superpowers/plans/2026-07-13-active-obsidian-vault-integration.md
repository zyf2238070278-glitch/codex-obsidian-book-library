...
```python
ACTIVE_OBSIDIAN_VAULT = "<OBSIDIAN_VAULT>"
```
...
```toml
env = { BOOK_LIBRARY_ROOT = "<PROJECT_ROOT>", BOOK_LIBRARY_OBSIDIAN_VAULT = "<OBSIDIAN_VAULT>", HF_HUB_OFFLINE = "1", TRANSFORMERS_OFFLINE = "1" }
```
...
def test_user_guide_uses_current_vault_paths_and_readable_spacing() -> None:
    ...
    assert "<OBSIDIAN_VAULT>" in guide
...
本项目已经连接到当前 Obsidian 仓库 `<OBSIDIAN_VAULT>`。不需要再选择 “Open folder as vault”，也不需要切换仓库；`书库` 会直接出现在当前左侧栏。原书位于 `书库/10-原始书籍/`，解析文本位于 `书库/20-解析文本/`。
```
...
- Create runtime directories only below `<OBSIDIAN_VAULT>/书库`.
...
```bash
sed -n '1,40p' ~/Library/Application\ Support/obsidian/obsidian.json
```

Expected: the open vault path is `<OBSIDIAN_VAULT>`.
...
BOOK_LIBRARY_ROOT="$PROJECT_ROOT" BOOK_LIBRARY_OBSIDIAN_VAULT="$OBSIDIAN_VAULT" HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 uv run python -c "import book_agent.mcp_server as s; print(s.library_tools.paths.vault); print(s.library_tools.paths.database)"
```
...
```bash
OBSIDIAN_VAULT="${BOOK_LIBRARY_OBSIDIAN_VAULT:?Set BOOK_LIBRARY_OBSIDIAN_VAULT first}"
find "$OBSIDIAN_VAULT/书库" -maxdepth 2 -type d -print
```
