...
- Active project: `<PROJECT_ROOT>`
- Source branch worktree: `<PROJECT_ROOT>/.worktrees/multi-engine-ocr`
- Active Codex config: `<PROJECT_ROOT>/.codex/config.toml`
- Existing book database and runtime data: `<PROJECT_ROOT>/data`
- Existing Obsidian Vault: `<OBSIDIAN_VAULT>`
...
**Files:**
- Inspect: `<PROJECT_ROOT>/.codex/config.toml`
- Preserve: `<PROJECT_ROOT>/AGENTS.md`
- Preserve: `<PROJECT_ROOT>/scripts/vision_ocr_pdf.swift`
- Inspect: `<PROJECT_ROOT>/data/library.sqlite3`
...
```bash
PROJECT_ROOT="$(git rev-parse --show-toplevel)"
/usr/bin/sed -n '1,120p' "$PROJECT_ROOT/.codex/config.toml"
```

Expected: `BOOK_LIBRARY_ROOT` is `<PROJECT_ROOT>` and `BOOK_LIBRARY_OBSIDIAN_VAULT` is `<OBSIDIAN_VAULT>`.
...
```bash
PROJECT_ROOT="$(git rev-parse --show-toplevel)"
OBSIDIAN_VAULT="${BOOK_LIBRARY_OBSIDIAN_VAULT:?Set BOOK_LIBRARY_OBSIDIAN_VAULT first}"
find "$PROJECT_ROOT/data" -type f -maxdepth 3 -print
find "$OBSIDIAN_VAULT/书库/10-原始书籍" -type f -maxdepth 1 -print
```
...
```bash
PROJECT_ROOT="$(git rev-parse --show-toplevel)"
git -C "$PROJECT_ROOT" merge-base main codex/multi-engine-ocr
git -C "$PROJECT_ROOT" merge-base --is-ancestor main codex/multi-engine-ocr
```
...
```bash
PROJECT_ROOT="$(git rev-parse --show-toplevel)"
git -C "$PROJECT_ROOT" merge --ff-only codex/multi-engine-ocr
```
...
PROJECT_ROOT="$(git rev-parse --show-toplevel)"
git -C "$PROJECT_ROOT" status --short
test -f "$PROJECT_ROOT/scripts/vision_ocr_pdf.swift"
```
...
**Files:**
- Generate: `<PROJECT_ROOT>/bin/book-vision-ocr`
- Generate: `<PROJECT_ROOT>/.venv`
- Generate: `<PROJECT_ROOT>/data/ocr-models/rapidocr/*.onnx`
- Rewrite with the same paths: `<PROJECT_ROOT>/.codex/config.toml`
...
PROJECT_ROOT="$(git rev-parse --show-toplevel)"
OBSIDIAN_VAULT="${BOOK_LIBRARY_OBSIDIAN_VAULT:?Set BOOK_LIBRARY_OBSIDIAN_VAULT first}"
python3 installer/install_macos.py --project-root "$PROJECT_ROOT" --vault "$OBSIDIAN_VAULT"
```
...
.venv/bin/python -m scripts.build_macos_release --project-root "$PROJECT_ROOT" --model-snapshot "$PROJECT_ROOT/data/models/models--intfloat--multilingual-e5-small/snapshots/614241f622f53c4eeff9890bdc4f31cfecc418b3" --uv-binary "$UV_BINARY" --vision-helper "$PROJECT_ROOT/bin/book-vision-ocr" --output-dir "$PROJECT_ROOT/dist"
```
...
```bash
PROJECT_ROOT="$(git rev-parse --show-toplevel)"
git -C "$PROJECT_ROOT/.worktrees/multi-engine-ocr" push origin codex/multi-engine-ocr
```
...
```bash
gh release edit v0.2.0-beta.1 --target codex/multi-engine-ocr --title "v0.2.0-beta.1｜双引擎本地 OCR" --notes "Apple Silicon Mac 测试版。Apple Vision + RapidOCR 本地回退、逐页跳过与 OCR 报告；压缩包内含 Word 安装说明和使用说明。"
```
...
```bash
gh release upload v0.2.0-beta.1 dist/codex-obsidian-book-library-v0.2.0-beta.1-macos-arm64-all-in-one.zip dist/SHA256SUMS --clobber
```
...
```bash
gh release view v0.2.0-beta.1 --json url,targetCommitish,assets
```
...
**Files:**
- Link: `<PROJECT_ROOT>/dist/codex-obsidian-book-library-v0.2.0-beta.1-macos-arm64-all-in-one.zip`
